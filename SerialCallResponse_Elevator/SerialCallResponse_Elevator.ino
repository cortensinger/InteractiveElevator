...
int firstSensor = 0;    // Potentiometer
int secondSensor = 0;   // Potentiometer
int thirdSensor = 0;    // Digital Sensor
int fourthSensor = 0;   // Photoresistor
int fifthSensor = 0;    // Flex Sensor
int sixthSensor = 0;    // Digital Sensor
int seventhSensor = 0;  // Digital Sensor
int eighthSensor = 0;   // Digital Sensor
int ninthSensor = 0;    // Digital Sensor
int tenthSensor = 0;    // Potentiometer
...
{ 
  ...
  if (Serial.available() > 0) {
    ...
    delay(10);
    fourthSensor = analogRead(A2);
    fourthSensor = map(fourthSensor, 500, 1100, 0, 255);
    delay(10);
    fifthSensor = analogRead(A4);
    fifthSensor = map(fifthSensor, 0, 300, 0, 255);
    sixthSensor = map(digitalRead(3), 0, 1, 0, 255);
    seventhSensor = map(digitalRead(4), 0, 1, 0, 255);
    eighthSensor = map(digitalRead(5), 0, 1, 0, 255);
    ninthSensor = map(digitalRead(6), 0, 1, 0, 255);
    delay(10);
    tenthSensor = analogRead(A3) / 4;
    delay(10);
    // send sensor values:
    ...
    Serial.write(sixthSensor);
    Serial.write(seventhSensor);
    Serial.write(eighthSensor);
    Serial.write(ninthSensor);
    Serial.write(tenthSensor);
  }
}
